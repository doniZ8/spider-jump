...
function updateHud() {
  ...
  if ($("inventory")) {
    $("inventory").innerHTML =
      (state.triple
        ? '<span class="pill">✦ Тройной прыжок</span>'
        : "") +

      (state.lives
        ? `<span class="pill">❤ Жизней: ${state.lives}</span>`
        : "") +

      (state.jetpack
        ? '<span class="pill">🚀 Джетпак активен</span>'
        : "");
  }
}
...
function resetWorld() {
  ...
  /*
    ВАЖНО:
    Персонаж сразу стоит на платформе.
    Никакого автоматического прыжка.
  */
  player.x =
    W / 2 - player.w / 2;
...
async function loadLeaderboard() {
  ...
  if (
    ...
    box.innerHTML =
      "<div class='row'>" +
      "<span>•</span>" +
      "<span>Онлайн-таблица не настроена</span>" +
      "<span>README</span>" +
      "</div>";
  ...
  try {
    ...
    box.innerHTML =
      "<div class='row'>" +
      "<span>!</span>" +
      "<span>Ошибка загрузки рейтинга</span>" +
      "<span></span>" +
      "</div>";
  }
}
